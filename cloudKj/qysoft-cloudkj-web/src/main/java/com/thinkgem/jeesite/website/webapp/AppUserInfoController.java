...
/**
    ...
    public String updateUserBank(UserUserinfoBank userUserinfoBank, HttpServletRequest request, HttpServletResponse response, Model model) throws ValidationException {
        ...
        try {
            ...
            return error ("认证状态读取失败", response, model);
        }
        if(renzheng>0){
...
//        userAccountchangePoundage.setOrderBy("a.create_date desc");
     ...
     * 提现详情
     */
    @RequestMapping(value = "/userTxDetail", method = RequestMethod.POST)
    public String userTxDetail(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if(StringUtils2.isNotBlank(id)){
            ...
            return error("失败:"+ "提现详情不存在",response,model);
        }
        userWithdraw = userWithdrawService.get(id);
        model.addAttribute ("poundage", Global.getOption("system_user_set","poundage"));
        model.addAttribute ("UserChargedetail", userWithdraw);
        return success ("成功!!", response, model);
    ...
    public String userinfo(HttpServletRequest request, HttpServletResponse response, Model model){
        ...
        model.addAttribute("least",Global.getOption("system_help","btc_withdraw_limt"));
        model.addAttribute("userLogo", Global.getOption("system_help", "user_logo"));
