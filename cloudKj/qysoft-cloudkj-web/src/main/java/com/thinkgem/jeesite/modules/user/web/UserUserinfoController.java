...
import com.thinkgem.jeesite.modules.user.service.UserUserinfoExtYkjService;
...
public class UserUserinfoController extends BaseController {
    ...
    @Autowired
    private UserUserinfoExtYkjService userUserinfoExtYkjService ;
    @ModelAttribute
    ...
    public String userCharge(UserChargeValidate chargeValidate, Model model, RedirectAttributes redirectAttributes) throws ValidationException {
        ...
        try {
            ...
            }else if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.money6.toString())){

                userUserinfoExtYkjService.chargeBtc(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), EnumUtil.MoneyType.money6, chargeValidate.getCommt()+"-系统充值" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.none_log);
            }else if(chargeValidate.getChargeType().equals(EnumUtil.MoneyType.money5.toString())){

                userUserinfoService.updateUserOtherMoney(chargeValidate.getUserName(), new BigDecimal(chargeValidate.getChargeMoney()), EnumUtil.MoneyType.money5, chargeValidate.getCommt()+"-系统充值" + UserUtils.getUser().getLoginName(), EnumUtil.MoneyChangeType.expend);
            }
            else{
