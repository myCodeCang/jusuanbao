...
var stepBar = {
    ...
    //this.itemWidth  tr的宽度
    stepInfoWidthFun : function(){
        if(this.itemCount > 0){
            this.barWidth = this.bar.width(); //屏幕宽度
            this.itemWidth = Math.floor((this.barWidth * 0.8) / (this.itemCount - 1)); //tr宽度
            this.bar.find(".ui-stepLayout").width(Math.floor(this.barWidth * 0.9 + this.itemWidth));
            this.item.width(this.itemWidth);
            this.bar.find(".ui-stepLayout").css({"margin-left": -Math.floor(this.itemWidth / 2) + 15 }); //ui-stepLayout向左移的值
            if(this.change){
                this._event();
            }
            this.percent();
    ...
    _animate : function(){
        ...
        if(this.triggerStep < this.curStep){
            stepSequence_size.padding = "4px 8px";
