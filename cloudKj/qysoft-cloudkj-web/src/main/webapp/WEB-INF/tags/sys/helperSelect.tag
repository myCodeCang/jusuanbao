...
<div class="append">
	<input id="helpedId" name="helpedId" class="${cssClass}" type="hidden" />
	...
	受助金额： &nbsp;&nbsp;<input type="text" id="helpedMoney" readonly="readonly" value="">
</div>
<script type="text/javascript">
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/help/hcHelpedPool/selectHelpedTagList?pageSize=8s", "选择${title}", 750, 500, {
			ajaxData:{selectIds: ""},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				if (v=="ok"){
                    ...
                    var money =  $(table).find(".selected").attr('dataMoney');
                    var id =  $(table).find(".selected").attr('dataId');
					$("#${id}").val(username);
					$("#helpedMoney").val(money);
                    $("#helpedId").val(id);
				}
				else if (v=="clear"){
					$("#${id}").val("");
